...
<div style="margin-bottom: 1rem;">
          ...
          <span
            style="margin-left: 1rem; font-weight: 500;
              color:
                ...
                : testStatus.startsWith('⚠️') ? '#f9a825'
                : '#666'};"
          >{testStatus}</span>
        {/if}
    </div>
    <div style="font-size: 0.85rem; color: #888; margin-top: 0.25rem;">
        <span>
            <strong>Note:</strong> If "Remember this key" is checked, your API key is stored only in your browser's local storage and never sent to our servers. Uncheck to remove it from storage.
        </span>
    </div>
</div>
...
It's like a backstage cue for the model to act a certain way.`,
        ...
        },
        {
            ...
            definition: 'If the prompt exceeds this many tokens, truncate older messages. Helps fit context limits.',
